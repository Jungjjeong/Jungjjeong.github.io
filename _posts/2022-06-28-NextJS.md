...
따라서 nextJS는 React에 필요한 여러 도구, 구성들을 처리하고 어플리케이션에 필요한 추가 구조나 최적화를 지원한다.
...
아직 Next.js를 능숙하게 사용해본 것이 아니라, 부족한 글이 되었다고 생각한다. 그리고 역시 공식 문서가 짱이다..! 😊 역시 만든 사람이 작성한 문서가 가장 정확하다.

...
따라서 for문 내에서 새로운 스코프를 갖기 때문에 매 루프마다 클로저가 생성됩니다.

> for문이 한번 반복될때마다 안에서 let 변수가 계속 새로 생성된다고 이해하면 될 것 같다!
> 
...
간단히 말해서 var 쓰면 안되는 이유가 바로 ‘호이스팅’입니다. 
...
```
  ...
  → 네 실행됩니다! 함수 선언이 맨 위로 끌어올려지기 때문이죠. 

## 주의

- 코드의 가독성과 유지보수를 위해 호이스팅이 일어나지 않도록 합니다.
    - 스코프 꼬임 현상이 일어나기 쉽습니다
- var을 쓰면 혼란스럽고 쓸모없는 코드가 생길 수 있습니다.

→ 하지만 우리가 var와 호이스팅에 대해 공부하는 이유는, ES5로 트랜스컴파일을 해야하기 때문이죠. ES6를 어디에서든 쓸 수 있으려면 아직 시간이 필요하기 때문입니다.

...
**선형 로그시간((O(n log n))과 이차 시간(O(n^2))**은 단순히 두 시간을 곱한 값이 각각의 시간이 됩니다. (선형 * 로그 / 선형 * 선형)
...
f(n) = O(h(n))이고 g(n) = O(p(n))이면 f(n) * g(n) = O(h(n)) * O(p(n))이다.  (곱해질 수 있다.)
...
하나의 반복문에서 i가 n * n * n까지라 하면 O(n^3)으로 표기할 수 있습니다.
